...
const getRecordsByDoctor = async (req, res) => {
  try {
    ...
    if (!doctor) {
      return res
        .status(404)
        .json({ success: false, message: "No doctor Available." });
    }
    const doctorRecord = await MedicalRecord.find({ doctor: doctor._id });
    if (doctorRecord.length == 0) {
      return res
        .status(200)
        .json({ success: false, message: "Doctor record not Available." });
    }
  ...
  try {
    ...
    if (!record)
      return res.status(404).json({
        success: false,
        message: "Medical record not found or deleted",
      });
